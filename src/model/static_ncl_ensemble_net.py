...
from src.model.static_ncl_base_learner import StaticNCLBaseLearner
...
class StaticNCLEnsembleNet:
    ...
    def train_base_learners(self, rank: int, data_loader_train: torch.utils.data.DataLoader, data_loader_validation: torch.utils.data.DataLoader, epoch_num: int, learning_rate: float, loss_function: any, lambda_ncl: float, epoch_num_per_log=10, validation_cycle=50, validation_epoch_window=500):
        assert validation_epoch_window % validation_cycle == 0, f"Expect validation_epoch_window % validation_cycle == 0 but: {validation_epoch_window} % {validation_cycle} = {validation_epoch_window % validation_cycle}"
        ...
        for device_index in range(base_learner_num):
            ...
            base_learner = StaticNCLBaseLearner(
                model=model,
                device=device,
                rank=rank,
                ...
                correlation_penalty_coefficient=lambda_ncl,
        ...
        for epoch in range(epoch_num):
            ...
            for batch_X, batch_y in data_loader_train:
                ...
                for i in range(len(base_learners)):
                    ...
                    base_learners[i].train_base_learner(
                        ...
                        y_true=batch_y,
                        y_prediction_mean=y_prediction_mean.to(device),
                        # NOTE: We will do the saving -model later
                        save_model=False
                    )
            ...
            # Use validation dataset to see if we should terminate training early
            if epoch % validation_cycle == 0:
                target_validation_loss_index = epoch // validation_cycle - \
                    validation_epoch_window // validation_cycle

                validation_loss_to_be_compared = None

                if target_validation_loss_index >= 0:
                    validation_loss_to_be_compared = validation_losses[target_validation_loss_index]

                terminate_training, lowest_validation_loss, current_validation_loss = self.validate_base_learners(
                    device=device,
                    base_learners=base_learners,
                    data_loader_validation=data_loader_validation,
                    loss_function=loss_function,
                    validation_loss_to_be_compared=validation_loss_to_be_compared,
                ...
                if terminate_training:
                    ...
                    # Log
                    print(
                        f"Training is stopped early at epoch {epoch + 1} / {epoch_num}")

                    break

        self.clean_up()

    def validate_base_learners(self, device, base_learners: list, data_loader_validation: torch.utils.data.DataLoader, loss_function: any, validation_loss_to_be_compared: float | None, lowest_validation_loss: float | None) -> tuple:
        """
        Perform early stopping on training if applicable
        Will save model

        :param: validation_loss_to_be_compared: Validation loss at validation_epoch_window epoches ago -> to determine whether we stop the training
        :param: lowest_validation_loss: Lowest validation loss -> we save the current model if the current validation loss is lower than the lowest validation loss
        :return: (terminate_training, lowest_validation_loss, validation_loss). terminate_training: True -> should terminate training; False -> continue training
        """
        # Test model using validation cycle
        batch_losses = []
        base_learner_num = len(base_learners)

        for batch_X, batch_y in data_loader_validation:
            # .to(device) returns a copy of tensor as it goes from CPU to GPU, i.e. different device
            batch_X = batch_X.to(device)
            batch_y = batch_y.to(device)
            ...
            # Calculaet mean prediction: shape [batch_num, 1]
            y_prediction_mean = global_y_predictions.mean(dim=0).mean(dim=0)

            batch_loss = loss_function(y_prediction_mean, batch_y)

            batch_losses.append(batch_loss)

        validation_loss = torch.mean(torch.tensor(batch_losses), dim=0)

        if validation_loss_to_be_compared is not None and validation_loss_to_be_compared < validation_loss:
            # Terminate training -> validation error has risen in comparison to validation_epoch_window epoches ago
            return (True, lowest_validation_loss)

        if lowest_validation_loss is None:
            lowest_validation_loss = validation_loss

        if validation_loss <= lowest_validation_loss:
            # Save model
            for base_learner in base_learners:
                base_learner.save_model()

            lowest_validation_loss = validation_loss

        return (False, lowest_validation_loss, validation_loss)

    def train(self, data_loader_train: torch.utils.data.DataLoader, data_loader_validation: torch.utils.data.DataLoader, epoch_num: int, learning_rate: float, loss_function: any, lambda_ncl: float, epoch_num_per_log=10, validation_cycle=50, validation_epoch_window=500) -> None:
        """
        ...
        mp.spawn(self.train_base_learners, args=(
            data_loader_train, data_loader_validation, epoch_num, learning_rate, loss_function, lambda_ncl, epoch_num_per_log, validation_cycle, validation_epoch_window), nprocs=self.__device_num, join=True)

    def evaluate_base_learners(self, rank: int, X: torch.Tensor, queue_y, lambda_ncl: float) -> None:
        """
        ...
        for device_index in range(base_learner_num):
            ...
            base_learner.load_state_dict(torch.load(
                f"{base_learner.model_repository_path}/base_learner_static_ncl_{rank}_{device_index}.pt"), model_configurations=self.__model_configurations)
    ...
    def evaluate(self, X: torch.Tensor, lambda_ncl: float) -> torch.Tensor:
        """
        Testing
        ...
        :param: lambda_ncl: Correlation_penalty_coefficient 
        ...
        mp.spawn(self.evaluate_base_learners, args=(X, queue_y, lambda_ncl),
    ...
    def evaluate_all_predictions(self, X: torch.Tensor, lambda_ncl: float) -> torch.Tensor:
        """
        For experiment
    ...
    def __call__(self, *args, **kwargs) -> torch.Tensor:
        ...
        :param: kwargs: Expect contain {"lambda_ncl": float} only
        ...
        assert "lambda_ncl" in kwargs, f"Missing required positional argument (float): lambda_ncl"

        return self.evaluate(X=args[0], lambda_ncl=kwargs["lambda_ncl"])
