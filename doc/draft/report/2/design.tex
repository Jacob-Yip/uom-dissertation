...
MSE was the primary metric for evaluating model performance. While certain experiments (e.g. NCL study in Section~\ref{subsec:stage_3}) incorporated a penalty term into the training loss, MSE remained the consistent evaluation standard for several reasons: 
\begin{enumerate}
    \item \textbf{Task Alignment} \\
    A regression data was used, for which MSE is the standard performance metric. 
    ...
    The differentiability of MSE is particularly advantageous for NCL. With the appropriate $\lambda$, the MSE derivative can simplify the NCL loss function by canceling specific components of the penalty term, resulting in a simpler derivative representation (refer to Theorem~\ref{theorem:loss_function_ncl_lambda_1}). 
\end{enumerate}
...
To ensure flexibility, all experiments were conducted in two stages. The first stage generated experimental data and outputted a \texttt{csv} file while the second stage produced the corresponding graphs from this output. This design enabled graph generation without re-running experiments. Experiment graphs were generated using the \texttt{plot} function from the Python visualisation library \texttt{Matplotlib}. 
...
All subsequent experiments used the same dataset, with MLPs as base learners. This stage implements two PyTorch models, a MLP and an arithmetic-mean-aggregator (a traditional ensemble model). 

The first part of this stage validates the MLP implementation and identifies key hyperparameters, e.g. number of training epochs. This ensures the foundational setup is correct for later experiments. 

The second part introduces a committee-based approach, whose codebase will be modified in Section~\ref{subsec:stage_3}. A single MLP is often unreliable because it may fail to fully capture the data patterns regardless of training time. In real life, critical decisions are usually made by a committee rather than one person. By applying this to machine learning, multiple models can "average out" individual errors. With the right aggregation method, the final prediction becomes more reliable and less sensitive to noise. 
...
\begin{enumerate}
    ...
    The input tensor is broadcasted to every base learner and each learner calcualtes their predictions simultaneously. 
...
\paragraph{Experimental Design for traditional ensemble model}
...
\paragraph{Motivations}
In Section~\ref{subsec:stage_1}, model configurations relied on passive diversity. While Theorem~\ref{theorem:ambiguity_decomposition} and Theorem~\ref{theorem:bias_variance_diversity_decomposition} establish that diversity is essential for ensemble performance, traditional techniques like bagging and boosting (as noted in Section~\ref{subsec:background_ncl}) only encourage this diversity indirectly. 

This stage investigates whether we can take a more proactive approach: is it possible to actively penalise base learners for being too similar to one another? However, excessive focus on diversity can be counterproductive, potentially degrading ensemble performance. Thus, an optimal balance between individual learner accuracy and collective diversity must be achieved. \textbf{Negative Correlation Learning} is explored as a mechanism for more granular control over this trade-off. 
...
The experiment on NCL ensemble model was based on the NCL experiments on the Boston Housing dataset conducted in Brown et al.\ \cite{brown2005managing}. It aimed at reproducing similar experiment results to study the effect of correlation penalty coefficient (represented as $\gamma$ in the paper Brown et al.\ \cite{brown2005managing}) on NCL ensemble loss. To extend the experiment, three extensions had been added to the NCL experiment in this work. 

In order to better compare the performance of NCL ensemble model and the original performance, MSE of a control MLP had been added. Although varying $\lambda$ did not affect the performance of the control MLP and the traditional ensemble model (from Section~\ref{subsec:stage_2}), having a control MLP and a control traditional ensemble model enable the visualisation of the performance improvement of NCL ensemble models at different values of $\lambda$. Furthermore, as mentioned in Brown et al.\ \cite{brown2005managing}, the upper bound of $\lambda$ could be larger than $1$ if the ensemble size was small enough (e.g. $\lambda = 2$). Thus, the NCL experiment extended the range of $\lambda$ from $[0, 1]$ to $[0, 1.5]$, allowing us to study the NCL ensemble performance for $\lambda$ slightly beyond $1$. Lastly, there were 2 ways of varying the model complexity. In Brown et al.\ \cite{brown2005managing}, model complexity was increased by adding more hidden nodes to the neural network but each neural network always had 1 hidden layer. In this project, model could get more complex by either increasing the number of hidden nodes (then all models always had 1 hidden layer) or the number of hidden layers (then always 2 hidden nodes for each hidden layer). In the reference experiments, early stopping is configured. In the current experiments, it never happened. 
...
\paragraph{Motivations}
In Section~\ref{subsec:stage_3}, diversity was explicitly enforced through NCL, under the assumption that all base learners shared a uniform architecture (MLPs with identical topologies and hidden nodes). In that context, diversity was derived solely from variations in weights. 

This stage moves beyond weight-based variation to explore the impact of \textbf{architectural diversity}. We investigate how structural differences between base learners affect overall ensemble performance and aim to identify the optimal architecture for this specific problem. Additionally, since Section~\ref{subsec:stage_3} demonstrated that the optimal penalty parameter ($\lambda^*$) varies across different models, we address the challenge of parameter selection and determine $\lambda$ dynamically during training rather than through exhaustive manual tuning. In this stage, we integrate NEAT with NCL to evolve diverse architectures alongside weights. 
...
\begin{enumerate}
    ...
    For each 2 genomes within a \textbf{subpopulation} (a group of genomes with the same number of hidden nodes), calculate their output distance $output\_distance_{ij}$ using the equation below: 
...
Two assumptions have been made. There must not be recurrent connections, i.e. the network must be \textbf{feed-forward}. Furthermore, the activation function used must be \textbf{idempotent} because the algorithm expects the values of input data remain unchanged when passing every node until they reach the last connection with weight~$w$. The activation function \texttt{torch.nn.ReLU} was acceptable only when the input data are \textit{non-negative}. This algorithm was not used in the experiment because it does not support backpropagation. The \textit{identity function} is also acceptable for this algorithm. 
...
\paragraph{Experimental Design}
Experiments had been conducted to compare the performance of a NEAT-NCL model with a control traditional ensemble model, a control NCL model and a control MLP. Table~\ref{table:experiment_4_varied_hyperparameters} contains all varied hyperparameters. Table~\ref{table:experiment_4_fixed_hyperparameters} contains all fixed hyperparameters. Table~\ref{table:experiment_4_metric} contains all evaluation metrics collected in this experiment. For raw fitness and adjusted fitness, the larger their values, the better the genome is. 
...
\paragraph{Motivations}
In Section~\ref{subsec:stage_2}, Section~\ref{subsec:stage_3} and Section~\ref{subsec:stage_4}, an arithmetic-mean-aggregator was employed to combine base learner predictions. Under this approach, every learner is treated with equal importance, contributing an identical weight to the final ensemble output. Inspired by \textit{weighted majority voting}~\cite{littlestone1994weighted}, this raises a critical question: how is performance affected if base learners are assigned varying degrees of importance? Beyond simple averaging, alternate aggregation strategies may offer improved robustness. For instance, a median-aggregator could better mitigate the impact of data skewness or outliers. 

In this stage, we compare the performance of three distinct aggregator types: an \textbf{arithmetic-mean-aggregator}, a \textbf{median-aggregator} and a \textbf{neural-network-aggregator}. When base learners are trained independently, using a neural-network-aggregator is equivalent to classical \textit{Stacking}~\cite{wolpert1992stacked}~\cite{breiman1996stacked}. The results from this analysis also serve as a baseline for Section~\ref{subsec:stage_6}, where we investigate how specific ensemble architectures interact with different aggregation mechanisms. 
