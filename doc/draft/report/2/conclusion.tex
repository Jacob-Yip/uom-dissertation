...
This project provides a comprehensive empirical and theoretical exploration of diversity and aggregation in ensemble learning, with a specific focus on \textbf{Negative Correlation Learning} (NCL) and its extensions. The primary contributions are as follows: 
\begin{enumerate}
...
All experiments were conducted using the \texttbf{Boston Housing dataset}, a standard regression benchmark. While the observed behaviours are inherently tied to this specific data distribution, future work can involve testing datasets with diverse characteristics, such as varying sample sizes and problem types, to uncover broader ensemble trends and effects of diversity. Additionally, removing the regression constraint would allow for the exploration of different aggregation mechanisms, such as \textbf{majority voting} for classification tasks. 

\paragraph{Double descent in ensemble MSE and $\rho$}
As mentioned in Section~\ref{subsec:evaluation_ncl}, an interesting and unexpected phenomenon of double descents in ensemble MSE and $\rho$ had occured when $\lambda$ reached extreme values. Further empirical studies to explain such behaviour could be attempted. 
...
Recursive ensemble models composed of different types of base learners could be explored with additional time. For example, one base learner in the recursive model could be a traditional ensemble model while the two base learners could be a NCL ensemble model and a recursively NEAT-NCL ensemble model respectively. 
...
The findings demonstrate that diversity is a double-edged sword: when carefully managed, it enhances predictive accuracy while reducing resource requirements; however, if left uncontrolled, an ensemble can actually underperform relative to a single model. The project successfully achieved the following initial objectives: 
\begin{enumerate}
    \item \textbf{Develop Baseline Models} \\
    A standard MLP and an arithmetic-mean aggregator were correctly implemented using PyTorch. The empirical observations aligned with theoretical expectations, confirming the robustness of the baseline environment. 

    \item \textbf{Implement NCL Ensemble} \\
    The implementation of a NCL ensemble allowed for a clear visualisation of the relationship between diversity and ensemble MSE. By adjusting parameters such as $\lambda$ and $m$, we successfully controlled diversity levels, providing empirical support for the theoretical frameworks (e.g. \textbf{Ambiguity Decomposition}) discussed in Sections~\ref{subsec:background_ensemble_learning} and~\ref{subsec:background_ncl}. 

    \item \textbf{Integrate NEAT with NCL} \\
    By combining NEAT with NCL, we explored the impact of \textbf{architectural diversity} on performance. This integration allowed for the evolutionary discovery of optimal model structures for the regression task and led to the development of a novel algorithm that enhances inference performance of dynamic neural networks. 

    \item \textbf{Analyse Different Aggregators} \\
    We implemented and compared three distinct aggregator types, highlighting how different approaches to combining predictions affect overall ensemble learning outcomes. 

    \item \textbf{Study Different Ensemble Topologies} \\
    This project introduced the concept of \textbf{recursive ensemble learning}. By comparing various recursive architectures, we identified the most effective combinations of aggregators and structural arrangements for this regression task, establishing a new direction for future research. 
\end{enumerate}